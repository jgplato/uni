...
\begin{enumerate}[a)]
  ...
  \item $\phantom{.}$
    ...
    \addplot[
        ...
        {4*(1/52.9)^3*exp(-2*x/52.9)*1/0.274981};
    \addplot[
        domain=0.1:1000,
        samples=90,
        line width = 1pt,
        ]
        {(1/2*sqrt(2)*(1/52.9)^(3/2))^2*(2-x/52.9)^2*exp(-2*x/(2*52.9))*1/0.274981};
      \node at (axis cs:100,0.000002) [pin={40:$1s$},inner sep=0pt] {};
      \node at (axis cs:250,0.0000008) [pin={40:$2s$},inner sep=0pt] {};
    \end{axis}
    \end{tikzpicture}
  \item Die größte Aufenthaltswahrscheinlichkeit findet sich bei $r=0$, direkt am Proton.
  \item Der Nullpunkt liegt bei
    \begin{align}
      &R_{2,0}(r) \overset{!}{=} 0 \\
      \Leftrightarrow\quad &2 - \frac{r}{a} = 0 \\
      \Leftrightarrow\quad & r = 2 \\
    \end{align}

    wobei $a$ der reduzierte Bohrsche Atomradius ist, also ca. bei $105{,}8 \; \pico\meter$.
\end{enumerate}
\end{document}
