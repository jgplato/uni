...
Induktionsvoraussetzung: Für die Anwendung des Satzes von Rolle auf $a_0(x)$
prüfen wir zuerst die wichtigen Vorraussetzungen:
...
dass zwischen diesen Nullstellen nach dem Satz von Rolle insgesamt $n-1$
Nullstellen liegen.  Die ersten beiden Vorraussetzungen haben wir bereits für
alle $a_n(x)$ überprüft.

Da $h_{n+1}(x)$ ein Polynom vom Grade $n+1$ ist, kann es zusätzlich zu diesen
$n-1$ noch maximal zwei weitere reelle Nullstellen haben.
...
Das bedeutet, dass die Funktion $a_n(x)$ an den Nullstellen $a_n(x_0)$ und
$a_n(x_n)$ ihr Vorzeichen wechselt, da die Funktion aber für $x \to \infty$
gegen $0$ geht, muss $a_n(x)$ nochmal die Steigung $0$ haben. Die Steigung $0$
bedeutet aber, dass an dieser Stelle (nennen wir sie $b$) $a_n'(b) = 0$ gelten
muss. Somit hat $a_n'(x)$ noch zwei weitere Nullstellen.

Also hat $a_{n+1}(x)$ genau $n+1$ reelle Nullstellen.
\end{document}
