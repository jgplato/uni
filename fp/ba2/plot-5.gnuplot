...
I0=25000
I(x) = I0*exp(-mu*x)
fit I(x) "data-5.txt" using 1:2:3 via I0, mu
I_err_n(x) = I0*exp(-(mu-mu_err)*x)
I_err_p(x) = I0*exp(-(mu+mu_err)*x)

set label 1 sprintf("Bestimmung des Absorptionskoeffizienten von Kupfer\n\nFitgleichung: $I(x) = I_0 \\exp \\left( - \\mu x \\right)$\n\n$I_0 = \\num{%1.6f+-%1.6f}$\n$\\mu = \\num{%1.6f+-%1.6f}$", I0, I0_err, mu, mu_err) at graph 0.07, graph 0.93

plot "data-5.txt" using 1:2:3 w yerrorbars ls 1, I(x) ls 2, I_err_p(x) ls 3, I_err_n(x) ls 3
