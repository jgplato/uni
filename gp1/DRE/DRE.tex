...
Da hier eine Drehbewegung auftritt ist es sinnvoll mit einem Ansatz für die
virtuelle Arbeit zu rechnen. Einerseits wird natürlich Arbeit verrichtet, wenn
sich die Masse im Gravitationspotential nach unten bewegt, andererseits ist für eine
Winkeländerung ein Drehmoment erforderlich, somit

\begin{equation}
  \delta W = mg \delta x + M \delta \phi = (mgr+M) \delta \phi
\end{equation}

Somit ergibt sich die generalisierte Zwangskraft (hier also ein Drehmoment)
\begin{equation}
  Q = mgr+M
\end{equation}

Statt eine Dissipationsfunktion zu nutzen können wir an dieser Stelle die
Zwangskraft auch direkt als Drehmoment angeben. Die Begründung des Ansatzes ist in Aufgabe 2
dargestellt. % FIXME Achtung: Und muss da auch bleiben!
...
\begin{equation}
  (mr^2+I) \ddot{\phi} = mgr + M - \mu \dot{\phi} \Leftrightarrow
  \frac{mr^2+I}{mgr+M} \ddot{\phi} = 1 - \frac{\mu}{mgr+M} \dot{\phi}
\end{equation}

Wir definieren
\begin{equation*}
  A = \frac{mr^2+I}{mgr+M} \quad B = \frac{\mu}{mgr+M} \quad \omega = \dot{\phi}
\end{equation*}
...
\begin{equation}
  A = \frac{(mgr+M)(mr^2+I)}{\mu^2} \qquad
  B = \frac{\mu}{mr^2+I} \qquad
  C = \frac{mgr+M}{\mu}
\end{equation}

Und können dagegen fitten
