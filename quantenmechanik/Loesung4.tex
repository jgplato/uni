...
\begin{enumerate}[a)]
  \item
    We are looking for the values of $c_1, c_2$ in
    \begin{equation*}
      \phi(x) = c_1c_2 \int\limits_{-\infty}^{\infty} dk e^{-ikx} \int\limits_{-\infty}^{\infty} dx e^{ikx} \phi(x)
    \end{equation*}

    Because we know that $c_1, c_2$ are constant, it is sufficient to
    sufficient to calculate it for a single function. We choose $\phi(x) =
    e^{-\frac{x^2}{2}}$ which should be invariant under Fourier transformations.

    \begin{equation*}
      \phi(x) = c_1c_2 \int\limits_{-\infty}^{\infty} dk e^{-ikx} \int\limits_{-\infty}^{\infty} dx e^{ikx} e^{-\frac{x^2}{2}}
    \end{equation*}
\end{enumerate}
