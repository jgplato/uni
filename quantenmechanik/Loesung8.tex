...
\begin{enumerate}[i)]
  \item
    Because $b$ and $b^\dagger$ satisfy the same commutation relations as $a$
    and $a^\dager$, we can write according to (3.61) and (3.62) of the script.
    If $N\ket{\xi} = \eta \ket{\xi}$, then
    \begin{align*}
      N b \ket{\xi} &= \del{bN -b} \ket{\xi} = \del{\eta - 1} b \ket{\xi} \\
      N b^\dagger \ket{\xi} &= \del{b^\dagger N -b^\dagger} \ket{\xi} = \del{\eta + 1} b \ket{\xi} \\
    \end{align*}
  \item
    Because the same relations apply, this is trivial.
\end{enumerate}
